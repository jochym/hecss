...
def _(monitor_phonons):
    ...
    T_1 = 3000
    supercell = '2x2x2'
    monitor_phonons(directory=f'example/VASP_3C-SiC_calculated/{supercell}/phon/', dfset=f'../T_3000K/DFSET.dat', kpath='3C_SiC', charge='3C_SiC', sc=f'../sc/CONTCAR', order=1, cutoff=10, born=2, k_list=None, once=True)  # Change to 2 if you want to monitor cubic potential  # Interaction range in Bohr. Do not increase too much.  # Change to None to deactivate  # Born effective charges calculation  # Change to string with points (e.g. 'KX')  # to plot selected points  # Show the plot and exit
    return
